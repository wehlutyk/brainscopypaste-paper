...
If $q_a$ was sufficiently prominent when $q_b$ first appeared, we can safely assume that the first author of $q_b$ on the 21st based himself on $q_a$ as is shown in the top drawing of Figure~\ref{fig:substitution-temporal-binning}.
...
or accurate reproductions of the previous occurrences of $q_b$? \cam{là je distinguerais 3a [top] de 3b [middle \& bottom]} (options shown in Figure~\ref{fig:substitution-temporal-binning})?

\begin{figure}[h]
    \centering
    \def\svgwidth{\linewidth}
    \small
    \input{images/substitution-temporal-binning.pdf_tex}
    \caption{XXXXX}
