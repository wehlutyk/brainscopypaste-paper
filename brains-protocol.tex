...
\rk{Add number of words to cells in heatmap}
...
All in all, apart from Word frequency and despite some tendencies these results do not allow us to conclude either to an effect or to no effect of the selected psycholinguistic features on substitution susceptibility.
...
\begin{figure*}[!th]
    ...
    \TB{Apart from Word frequency (where low-frequency words are more susceptible than high-frequency words), results do not let us conclude either to an effect or to no effect of the features on susceptibility.
    The heatmap on the lower-right details the interaction of Age of acquisition and Number of phonemes on susceptibility; susceptibility values are indicated in the cells, and the number of words per cell is indicated in parentheses.
    Once outlier values with only 11 words are discarded, the heatmap shows no significant interaction effect.}}
...
In other words, beyond individual variation patterns, the substitution process is contractile for all the features, and each of them therefore exhibit a unique attractor.
Second, the comparison with $\mathcal{H}_0$ shows that there are two classes of attractors, depending on whether (i) there is a triple intersection (of $y=x$, $\nu_{\phi}^0$ and $\nu_{\phi}$) or (ii) $\nu_{\phi}$ remains always above or below $\nu_{\phi}^0$.
...
\TB{(furthermore, for these two features $\mathcal{H}_{00}$ stays inside the confidence intervals of the alteration, confirming that they are irrelevant for the substitution process)}.
\rk{Ajouter ce texte en bleu si tu décides de mettre les figures avec $\mathcal{H}_{00}$.}
The second class on the other hand (comprising Word frequency, Age of acquisition, and Clustering coefficient) are features for which the substitution process has a clear bias in one direction or the other.

Word frequency, with destination words always significantly above $\nu_{\phi}^0$, exhibits a strong bias towards more frequent words \TB{(this in turn confirms that substitution is more of a recall process than a familiarity process, since common words are favored over awkward ones)}.
\rk{Mais est-ce que c'est pas une inférence inverse que de dire ceci? i.e. on sait que "familiarity" implique un biais en faveur des mots awkards, et que "recall" implique un biais en faveur des mots fréquents, mais on peut pas vraiment tirer de conclusions en utilisant la réciproque, si?}.
The detailed variations of the Word frequency curve, despite an inflexion for high-frequency words, do not seem to follow any significant pattern.
...
Clustering coefficient has the additional particularity that, on average, the destination word does not depend on the start word: a substitution will always produce words with a clustering coefficient near $\exp(-2.4) \simeq 0.1$.

To make things concrete, here is an example substitution taking place in the dataset.
At the end of December 2008, many science-related websites reported the following quote,

\begin{quote}
    ``It could be that our emotions and the systems to regulate them are vestiges of our evolutionary ancestry.''
\end{quote}
and a minority of other websites reported the following,
\begin{quote}
    ``It could be that our emotions and the systems to regulate them are vestiges of our evolutionary \textbf{history}.''
\end{quote}
The word \emph{ancestry} is acquired at an average of 10.11 years old, appears about 12 times in the dataset, and (unfortunately) is not encoded in Free Association norms, meaning we have no Clustering coefficient for this word.
The word it was replaced with, \emph{history}, is acquired on average at 6.42 years old, appears about 33k times in the dataset, and has a Clustering coefficient of 0.1.
Such a change, though minor in appearance, is a typical example of alteration along the lines shown by our results.

%ancestry (age 10.11, phonemes 8.0, freq 2.48, cc -, susceptibility 1.0)
%history  [age 6.42, phonemes 6.0, freq 10.42, cc -2.30, susceptibility 0.0]
