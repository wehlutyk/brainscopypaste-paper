...
This is remarkably clear for Clustering coefficient and Age of acquisition, where susceptibility values remain within quite small intervals (respectively [0.13-0.18] and [0.16-0.20]\marginnote{corrigé les intervalles en fonction des arrondis (.5->1), à partir du graphique ceci dit (pas trouvé les valeurs exactes sur le ipython}).
{We may notice a slight effect for the lowest values of Number of synonyms and Number of phonemes, where the mean susceptibility is almost half as high as in the rest \marginnote{the rest c'est la moyenne des other values ou la plus extrême?} (respectively 0.\TB{0}9 vs 0.16, and 0.11 vs 0.17).}
...
\TB{TBF: [[We see that outlier values (the two darkest cells) are both due to cells where the number of items is only slightly above the significance threshold (11 items, while we decided to cut at 10).]]}
\TB{TBF: [[If we focus on cells based on at least several dozens of words]]} Values tend to navigate around the mean value \TB{[TBF:()]}, \TB{while we observe a few outlier cells with little obvious regularity (except for a low number of synonyms, consistently with the unidimensional graph).} \TB{On the whole, this makes it relatively hard to draw any conclusion as regards the direction of an effect, except for the least populated value ranges (which as a result are also less significant).}
\TB{TBF: [[This confirms the fact that the slight late increases observed for Age of acquisition and Number of phonemes are rather non-significant.]]}
...
All in all, apart from Word frequency and despite some tendencies, \TB{in general} these results do not allow us to conclude either to a marked effect %or to no effect 
