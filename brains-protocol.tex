...
We could therefore study the individual transformation process at work when authors alter quotations, by examining the modified words in each transformation. To keep the analysis palatable, we focused on quotation transformations consisting in the \emph{substitution} of a word by another word (and only those cases) in order to unambiguously discuss single word replacements.
...
\todo{\#17: go into more detail on how quotes are selected by Leskovec et al. (or explain there's no info if there's none)}
...
\todo{\#20: make clear we didn't do this, it's Leskovec et al.}
...
\todo{\#17: precision/recall analysis on anti-spam and language}
