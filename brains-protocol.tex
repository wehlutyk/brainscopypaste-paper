...
$q$ and $q''$ differ by two substitutions, so we ignore the possibility that one was written based on the other.\footnote{
NOTE WE TESTED THIS.
}
...
\begin{figure*}[h]
	...
	If the bins were extended to the beginning of the quotation family, the exclude past constraint would also remove the $q' \rightarrow q$ substitution from bin~2 to bin~3.
