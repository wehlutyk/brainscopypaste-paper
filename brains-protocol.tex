...
\begin{figure}[!th]
    ...
    \caption{Spearman correlations in the filtered set of features \rk{further remove FA betweenness?}}
...
The main question in this problem concerns the way we consider later occurrences of a quotation that has already occurred earlier: the author of the later occurrences could have sourced an earlier occurrence and accurately copied it, they could have transformed an earlier occurrence of \emph{another} quote into this one, or they could have relied on information or other quotes from outside the dataset, i.e. from an external source we do not measure.

Let us give an example: say the quotation ``These accusations are false and \textbf{absurd}'' ($q_a$) appears in a blog on January 18, and the slightly different quotation ``These accusations are false and \textbf{incoherent}'' ($q_b$) appears in other blogs twice on the 19th and once on the 20th of January.
If $q_a$ was sufficiently prominent when $q_b$ first appeared, we can safely assume that the first author of $q_b$ on the 19th based himself on $q_a$.
But what about the second occurrence of $q_b$ on the 19th, and the one on the 20th?
...
\begin{figure}[h]
    ...
    \small
    \input{images/substitution-detection-example1.pdf_tex}
    \caption{XXXXX}
...
\rk{We also restrict to words substituted at least once.}
...
\footnote{In this case $\warrival_{\mathcal{H}_{00}}$ does depend on $\wstart$.} \rk{Do we present these results?}\cam{nope}\rk{S: Then we must say the results are the same.}
