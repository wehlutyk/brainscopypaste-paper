...
The analysis showed a better prediction effect of the number of phonemes over the number of syllables, which is consistent with~\citet{nick-diss}, and we therefore chose to only present results for the former.\marginnote{C: which analysis, ours? (if yes, the notion of ``prediction effect'' wouldn't appear to be very understandable at this stage of the paper)} 
...
We also introduce a null hypothesis $\mathcal{H}_0$ to compare the actual variation of a word's feature to its expected variation, assuming the arrival word $\warrival$ has been chosen randomly from the whole pool of free association words.\marginnote{C: ici on est bien d'accord que le pool of FA words c'est tout le set de mots, pas juste les voisins? la formulation est un peu ambigüe}
