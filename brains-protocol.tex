...
\begin{figure}[!th]
    ...
    \caption{Correlations in the initial set of features \rk{show non-log version?} \cam{if we use log variables I guess we should show correlations on log variables -- I'm surprised by the level of correlation b/n (aoa) and (log PR), though}}
...
These various flavors of an ideal substitution detection model %not only
 have an impact on whether occurrences are considered as substitutions or repetition of the original quote. %, but also essentially modify how many times a given 

%variations essentially modify the balance between 

%considering the second blue quote as a reproduction of the first blue because the time window is too short 

%quote or as a mutation 

%taking into account the possibility of a 

%reproduction of previous occurrences of a quote and re-creation of itself by a new substitution instance.
