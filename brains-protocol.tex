...
To keep the analysis \newtext{tractable}, we focused on quotation transformations consisting \tb{of} the \emph{substitution} of a word by another word (and only those cases) in order to unambiguously discuss single word replacements.
...
To quantify \tb{these} substitutions we decided to associate a number of features to each word, the variation of which we can statistically study.

The next subsections describe the data set and the measures we used to assess this cognitive bias.

\subsection{\tb{Corpus-based} utterances}
...
\end{quote}
\newtext{Next, they applied a community detection algorithm} to that quotation graph to detect aggregates of tightly connected, \tb{i.e.} sufficiently similar, groups of quotations~\citep[see again][for more details]{leskovec_meme-tracking_2009}.
...
Even if we lose some real event-related utterances which are present in clusters lasting more than 80 days (one such lost quote, for instance, is ``the city is tired of me and the organization and I have run our course together''), we check that our \tb{filtering} approach fulfills its goals by coding a random sub-sample of 100 clusters: 35 of them are rejected by the filter, with 15 false negatives (rejected clusters that should have been kept) and 9 false positives (clusters kept when they should have been rejected), giving a precision score of 0.862 and a recall score of 0.789.
...
\newtext{To compute \tb{these} features} we relied on the free association (FA) norms collected by \citet{nelson_university_2004}, which record the words that come to mind when someone is presented with a given cue.
...
To make correlation values as well as future comparisons more reliable, we log-transformed features that have marked exponential distributions (a few words valued orders of magnitude higher than the vast majority of other words).}
%\footnote{
...
The final set of features we will discuss in the rest of the paper, as well as their cross-correlations, can be seen in Fig.~\ref{fig:feature-corrs-filtered}.% (the analysis on the complete set of features can be found in the appendix).
\footnote{
...
\tb{Here,} $q$ and $q''$ differ by two substitutions, so we discard the possibility that one was written based on the other (\tb{see} below for further details).
...
(2) \textbf{bin span}, which can be \tb{$\Delta t=1$ day} %24 hours 
(as in the model above) or can be extended to start at the very first occurrence in the quotation family.
The other two parameters \tb{configure} rules on the selection of source and destination quotes of a substitution:
...
Fig.~\ref{fig:substitution-modelled} shows the inferences made by the four models that use \tb{discrete} %ly positioned 
bins spanning 1 day:
...
In practice \newtext{for the model first introduced above}, from the \newtext{2.60m} initial occurrences spread \tb{over} 50,427 quotation families, with significant redundancy (many quotes are indeed simple duplicates), we mine \newtext{40,868} substitutions.
...
this eventually yields \newtext{6,318} valid substitutions\footnote{
...
out of the first 100 substitutions coded for recall, those with Levenshtein distance equal to 1 (which is what orthographic neighborhood density codes,~\citealp{marian_clearpond:_2012}) were all typos or UK/US spelling changes, neither of which are relevant for this study.}} \newtext{\tbn{(before merging substitutions that share the same destination)}}.

\section{Results}\label{sec:results}

\begin{new}

\tb{Substitutions usually} replace a word with another semantically related word: the new word achieves a similar meaning in the context of its sentence, while still slightly changing the implications or the attitude expressed by the author. The following examples illustrate this \tb{phenomenon}:
...
\tb{However, the substituted and subtituting words are not so often direct synonyms:} only a third of all substitutions travel less than 3 hops on the hyponym-hypernym network defined by WordNet (direct synonyms count as 0~hops on this network), meaning that at least two thirds involve non-synonyms.\footnote{
\newtext{A similar \tb{phenomenon} is observed on the FA network, where about 104 clusters have substitutions traveling only 1 hop, 110 traveling 2 hops, 137 traveling 3, 72 traveling 4, and 13 traveling 5.
}}
...
\newtext{Our question concerns the low-level properties of these substitutions: \tb{we ask (i) which words are targets of the substitutions and (ii) what change they are subjected to. To this end,}} we build the following two observables for each word feature.
...
\begin{figure}[h]
    ...
    \caption{\newtext{\textbf{\tb{POS-related results:}} categories are simplified from the TreeTagger tag set: \emph{J} means adjective, \emph{N}~noun, \emph{R}~adverb, \emph{V}~verb, and \emph{Stopword-like} gathers together all the categories partially or completely affected by the stopword filter (see main text for details).
    The top panel shows the actual $s_{POS}$ and $s_{POS}^0$ counts. The bottom panel shows \tb{the substitution susceptibility $\sigma_{POS}$, which is the ratio between the two previous counts}.
...
If, for instance, half the content words of a given quote are nouns, such a quote contributes .5 to the total $s_{nouns}^0$.} \tb{Further, t}o avoid possible autocorrelation effects due to substitutions belonging to the same cluster (which are likely not statistically independent and may lead to overly optimistic confidence intervals), we scale $s_g$ and $s_g^0$ to count one for each cluster.
That is, each individual substitution contributes to $s_g$ or $s_g^0$ by only 1 over the number of substitutions in its cluster.}
...
\tb{\hbox{i.e.} it measures the preferential selection of some word properties given a substitution.}
...
\newtext{This means that for each feature $\phi$, whichever the value $\phi(\wstart)$ of the disappearing word, the appearing word's feature value $\phi(\warrival)$ will, on average, be closer to that feature's intersection between $\nu_{\phi}$ and $y=x$.\footnote{
...
\begin{figure}[h]
    ...
    source feature values \tb{(columns)} and feature variations \tb{(rows)} were normalized to $[0; 1]$ to ensure the coefficients are comparable.
...
\newtext{All these effects are significant at $p < .05$ (and more often $p < .001$) and were verified across the 16 substitution models.}
