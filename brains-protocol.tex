...
Fig.~\ref{fig:substitution-modelled} shows the inferences made by the four models that use discrete %ly positioned 
bins spanning 1 day:
...
\tb{manual observation of a random subset of 100 substitutions shows that, compared to the word it replaces,} the new word often achieves a similar meaning in the context of its sentence while still slightly changing the implications or the attitude expressed by the author.
...
\newtext{Our question concerns the low-level properties of these substitutions: we ask (i) which words are targets of the substitutions and (ii) what change they are subjected to. To this end,} we build the following two observables for each word feature.
...
\begin{figure}[h]
    ...
    categories are simplified from the TreeTagger tag set: \emph{J} means adjective, \emph{N}~noun, \emph{R}~adverb, \emph{V}~verb, and \emph{Stopword-like} gathers together all the categories \tb{essentially made of stopwords} (see main text for details).
...
\tb{That is, each quote cluster has a maximum contribution of 1, computed as the average contribution of all substitutions in that cluster.}
}}
...
\hbox{i.e.} \newtext{it measures the preferential selection of some word properties \tb{for} substitution.}
...
\newtext{The \emph{Stopword-like} category gathers all the POS groups \tb{essentially made of stopwords (coordinating conjunctions, foreign words, prepositions, subordinating conjunctions, modals, possessive endings, punctuation symbols and interjections), which are affected by our decision to exclude stopwords from the analysis.
Indeed substitutions falling on stopwords are filtered out (affecting $s$), and stopwords are never counted as substitutable in the computation of $s^0$.
Both numbers are subsequently very small for these categories (which led us to group them together) but the reported susceptibility is left unbiased.}}
}
