...
\rk{Recognize that this can be a sub-sample of real human manual Ctrl-C/V, since we don't have access to successful manual copy by the brain. No need to suppose this sample is representative: we focus on when there is a transformation, and that's the process we study (since we have no access to the difference between manual and automatic). We also therefore suppose that manual copy and manual transformation are potentially different processes, and can be studied separately profitably.}
...
This class of words corresponds to \TB{highly specialized words? domain-specific? a hard limit on too complicated words?}

Finally the results for Clustering coefficient and Number of synonyms, despite some tendencies, do not allow us to conclude either to an effect or no effect of those features on substitution susceptibility.
