...
\begin{figure*}[!th]
    ...
    \TB{TO BE FIXED: Without the rather non-significant cells (11 words or less), the heatmap shows no specific trend in one direction or the other.}}
...
\TB{If we set aside the values for low Number of phonemes}, for each of these features it is indeed possible to draw a constant line which always remains within the respective confidence intervals.
If these variables have an effect, it is by no means as strong as it is for Word frequency.
This is remarkably clear for Clustering coefficient and Age of acquisition, where susceptibility values remain within quite small intervals (respectively [0.13-0.19] and [0.15-0.20]).
\TB{We may notice a slight effect for the lowest values of Number of synonyms and Number of phonemes, where the mean susceptibility is almost half as high as in the rest (respectively 0.9 vs 0.16, and 0.11 vs 0.17).}
Keeping in mind the poor statistical significance of this effect, we could still suggest that the shortest words and words with few synonyms are less susceptible to substitution.
\TB{TO BE FIXED: To further examine this phenomenon, we plotted the two-dimensional map of susceptibility values for these two features (see heatmap at the bottom right of Fig.~\ref{fig:feature-susceptibilities}).
...
All in all, apart from Word frequency and despite some tendencies these results do not allow us to conclude either to a marked effect %or to no effect 
