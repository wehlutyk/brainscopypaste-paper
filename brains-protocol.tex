...
The distributions of original and log-transformed features appears in the supplementary material, along with correlation scatter plots for each feature pair, summarized below with correlation values.
}}
...
say the quotation "These accusations are false and \textbf{absurd}" ($q$) appears in a blog on January 19, and the slightly different quotation "These accusations are false and \textbf{incoherent}" ($q'$) appears in other blogs twice on the 20th and once on the 21st of January.
...
or accurate reproductions of the previous occurrences of $q'$?
...
\begin{figure}[!h]
	...
	\subfloat[Binned quotation family with majority rule]{
	    ...
	    \label{fig:substitution-Time=discrete-Source=majority-Past=last_bin-Durl=all}
	}
	\hfill \\
	\subfloat[Binned quotation family with majority rule and excluded past]{
	    \def\svgwidth{\linewidth}
	    \small
	    \input{images/substitution-Time=discrete-Source=majority-Past=last_bin-Durl=exclude_past.pdf_tex}
	    \label{fig:substitution-Time=discrete-Source=majority-Past=last_bin-Durl=exclude_past}
	}
	\hfill \\
	\subfloat[Binned quotation family with \emph{no} majority rule and excluded past extended to the beginning of the quotation family]{
	    \def\svgwidth{\linewidth}
	    \small
	    \input{images/substitution-Time=discrete-Source=all-Past=all-Durl=exclude_past.pdf_tex}
	    \label{fig:substitution-Time=discrete-Source=all-Past=all-Durl=exclude_past}
	}
...
\todo{\#12: fix substitution counts and add precision/recall}
...
\todo{Note that our minor-spelling-changes exclusion does not affect orthographic nd: in a sample of 100 substitutions, no rejected substitutions are minor orthographic changes that would interest us (they're always different spelling of the same concept).}
