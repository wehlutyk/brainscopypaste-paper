...
\answer{We agree the presentation was too much. \unclear{too much?} Point to:
\itx{
\x the integration of the iterated learning literature in review and general discussion
\x the fact that the constraints of the data don't let us explore chains, so we explained our claims more appropriately
}}
...
\itx{
\x We checked the susceptibility of part-of-speech (POS), \unclear{je n'ai pas bien compris ton: ``showing that the effect (closed/open class words being different) expected is very small at best (closed class words would be stopword-likes in our graph)''  (et c'est très légèrement lié à mon point {\bf (5)} dans l'email) -- est-ce qu'il s'agit de l'effet des stopwords-like (== closed class words, exactement?) sur nos résultats (auquel cas, oui, ça a l'air de ne pas arriver souvent, en même temps on se focalise sur les substitutions) ou bien de la susceptibilité des stopwords-like par rapport aux autres (auquel cas elle semble effectivement très inférieure)}
\x Following this suggestion, we added phonological neighborhood density as a word feature. We show that it forms a subgroup of strongly correlated variables, together with orthographic neighborhood density, and choose the latter as a more natural representative of this subgroup given the task under study.
\x We introduced a multi-feature regression model to estimate variation upon substitution, showing that the magnitude of the variation of a feature is mainly predicted by the value of that very feature, generally with negligible interaction with the other features (see also below and figure 8).
\x We expanded substitution models to account for \emph{two-substitutions} \unclear{explain here what is a ``two-substitution'', maybe?}, which yield the same results as single substitutions. 
(Note: %computation notebooks for all models can be found at https://github.com/wehlutyk/brainscopypaste/tree/master/data/notebooks , and the 
graphs produced for all models may be found at <https://huit.re/aEH77Gc3>, and may be commented further upon request.)
}
...
\answerok{
These points should be addressed by the improved introduction. In particular regarding the last question, the new ``Discussion'' section additionally makes a connection with iterated learning literature.
}
...
\unclear{ah oui que répond-on là? qqch comme ça? je te laisse valider:}
...
\answer{That would indeed be interesting, but not our goal in the paper.\unclear{``merci vieux mais c'est hors-sujet'' ;) Pourrait-on lui dire plus diplomatiquement:} \textcolor{red}{``This would indeed be a very interesting project and prospect, but the data we collected do not enable the study of the surrounding context (in particular, we do neither know the prior usage nor the way information sources may be used) so that this goal remains beyond the scope of our paper.''}}
...
\answerok{This is an important empirical point which we did not address systematically in the first submission.  We now have done this manual coding step and are glad to be able to report a (satisfying) accuracy in the relevant cases.}
...
\answerok{\textcolor{red}{[C: j'ai ajouté ça, je te laisse confirmer si ce teasing te semble approprié]}\\ On a side note, we entirely agree with the relevance of pursuing this research program in these directions, as the \emph{in vitro} observation of information transmission and its evolution is part of an ongoing project (which nonetheless remains beyond the scope of the current paper).}
