...
We can therefore assume that most transformations, especially the simple ones, are the result of automatic (\hbox{i.e.} hard to control) low-level cognitive biases of the authors.
...
While this limits the scope of our observations to the particular data set we use, the methodological point we also make is left intact.
By characterizing words using 6 well-known features, we identify what makes a substitution more likely, and how a word changes when it is substituted.
...
Given this complexity we decided to focus on more aggregate effects, where variations of the conditions in which sentences are read and produced have a chance of being statistically smoothed out.\footnote{
