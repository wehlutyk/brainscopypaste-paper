...
\todo{
\#14: Rework to
(1) present shifts in biology (eco-evo-devo) rejoining attempts to bridge social/cognitive rift and study knowledge transmission (cultural evolution),
(2) present current problem of testing theories to incorporate those discoveries,
(3) present big data literature which can bring measurement tools,
(4) introduce the question of cultural attractors,
(5) to be studied through quotes on the web: it's the proposal of a novel approach, we'll be describing and cross-checking with existing literature (vs. predicting predict, since there are too many possible things to predict)
}
